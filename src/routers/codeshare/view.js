...
console.log("[mebot.ga]: Code Share/View router loaded.");
...
app.get("/view/:code", global.checkAuth, async (req, res) => {
    ...
    if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id)) return res.redirect("/error?code=403&message=To do this, you have to join our discord server. (If you are on the Server, Write the m-confirm command in the ✔・⌬〢confirm Channel!)");
